...
class CommentAnalyser(ABC):
    ...
    def analyse_comments(self):
        ...
        if len(self._files) == 2 and self._comments_file2 is not None:
            for comment in self._reviewable_comments:               
                comment_equivalent = self._get_comment_equivalent(comment)
                if comment_equivalent is not None:
                    comment.update_line_number(comment_equivalent.line_number())
        ...
        if len(self._files) == 2:
            ...
            if comments_file1 and comments_file2:
                ...
                if self._edit_script_actions:
                    ...
                    self._tree = source_to_tree(self._files[0])
    ...
    def _next_comment(self):
        if self._comment_index + 1 < len(self._comments):
            self._comment_index += 1
    
    def _get_current_comment(self):
        return self._comments[self._comment_index]

    def _is_task_comment(self, comment_text):
        for keyword in self.TASK_IDENTIFIERS:
            result = Utils.match_phrase(keyword)(comment_text)
            if result is not None:
                return True
        return False
    
    def _is_copyright_comment(self, comment_text):
        if self._is_valid_weblink(comment_text):
            return True
    ...
    def _is_commented_code(self, comment_text):
        ...
        if symbol_search_result is not None:
            # Allow web links of the form http(s)://www.example.com
            if symbol_search_result.group(0) == ":":
                if not self._is_valid_weblink(comment_text):
                    return True
            else:
                return True
        ...
        for regex in self._code_word_regexes:
            result = Utils.match_phrase(regex)(comment_text)
            if result is not None:
                return True
    ...
    def _find_deleted_names(self, comment_text):
        ...
        for name in self._deleted_names:
            result = Utils.match_phrase(name)(comment_text)
            if result is not None:
    ...
    def _find_refactored_names(self, comment_text):
        ...
        for v,k in self._refactored_names.items():
            result = Utils.match_phrase(v)(comment_text)
            if result is not None:
    ...
    def _check_comment(self):
        ...
        if current_comment in self._comments_file2:
            self._cosmetic_check(current_comment)
    ...
    def _check_relevance(self, entity):   
        current_comment = self._get_current_comment() 
        if current_comment in self._comments_file2:
            cosmetic_errors = self._cosmetic_check(current_comment)
    ...
    def _register_outdated_comment(self, cosmetic_errors):
        comment_equivalent = self._get_comment_equivalent(self._get_current_comment())
    ...
    def _get_comment_equivalent(self, comment):
        index = next((i for i, item in enumerate(self._comments_file2) if item.text() == comment.text()), -1)
        if index != -1:
    ...
    def _get_registered_comment(self):
        index = next((i for i, item in enumerate(self._reviewable_comments) if item.text() == self._get_current_comment().text()), -1)
